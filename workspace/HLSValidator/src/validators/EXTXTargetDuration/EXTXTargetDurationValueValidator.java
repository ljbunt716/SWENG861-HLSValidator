			...
			if (IsValidFloat(targetDurationValue)) {
				...
				for (Integer infValue : infValues) {
					...
					if (infValue > targetValue) {
						super.AddValidationError(
								"#EXT-X-TARGETDURATION value must be greater than or equal to #EXTINF value -  #EXT-X-TARGETDURATION = "
										+ targetValue + ", #EXTINF = " + infValue,
								"#EXTINF Value Greater than #EXT-X-TARGETDURATION Validation");
					}
				}
			}
